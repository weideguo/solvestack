...
|参数替换格式|替换后的字符值|
| :--- | :---- |
| <span v-pre>{{db2}}</span>             |server_db_192.168.253.128 |
| <span v-pre>{{db2.host}}</span>        |realhost_192.168.253.128  |
| <span v-pre>{{db2.host.ip}}</span>     |192.168.253.128           |
...
核心思想的是playbook的一行对应一个单行shell命令，可以使用<span v-pre>{{XXX}}</span> 格式的模板语言实现参数引入。  
...
只需要在主机管理中设置涉及的主机，可以动态配置执行信息与执行命令快速生成任务，playbook与常规任务格式类似（没有session参数与执行对象参数，增加分割参数如 <span v-pre>{{\_1}} {{\_2}} </span>，对应配置信息分割后的第一个参数、第二个参数），每一行配置信息一个并发（也可以选择串行执行）。
