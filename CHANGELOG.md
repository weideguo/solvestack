# 0.19.0 (2026-07-13)
### Added
  - permanent_token增删改查页面
  - 在`执行任务`时可以重新选择执行对象
  - 后端增加OpenAPI文档（debug模式时启用）
  - 后端增加主机执行接口

### Changed
  - 前端升级到vue3，使用vite构建项目，代码风格改成vue3格式
  - 前端所有依赖项升级至当前最新版本
  - 前端路由从vuex替换成pinia
  - 前端配置文件改成单独引入，方便编译后更改
  - 前端Dockerfile改成编译后使用nginx运行项目
  - 核心、后端使用black格式化代码；所有文件换行符统一为`LF`
  - 核心、后端日志文件按照日期分割
  - 使用vitepress展示说明文档
  - 放弃对python3.9以及之前版本的支持
...
### Changed
 - redis配置格式调整
 - 移除python-daemon
 - 支持python3.9、python3.11、python3.13，放弃对python3.7以及之前版本的支持
 - python依赖包升级至最新版本
 - 前端升级运行时，依赖包升级
 - 前端表单优化
...
### Added
 - `__sync__` 语法支持
 -  支持在playbook描述文件使用`target_type`字段确定执行对象名，前端可以根据此选择对象


# 0.15.4 (2024-09-29)
### Added
 -  支持在playbook描述文件使用`target`字段确定执行对象组成，前端可以根据此选择子对象构建执行对象
